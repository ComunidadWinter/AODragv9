...
If Err Then
    MsgBox "No se puede iniciar DirectDraw. Por favor ejecute DRAG-PROBLEMAS.EXE que se encuentra en la carpeta de AodraG para solucionar su problema, si tiene Windows Vista no olvide dar botón derecho de ratón y ejecutar como administrador."
...
toto:
'pluto:2.14
If Not FileExist(App.Path & "\fonts.exe", vbArchive) Then
MsgBox ("Error cargando tipos de letras, posiblemente le falte alguno de los archivos de fuentes de letras.")
End
End If



AddtoRichTextBox frmCargando.status, " DirectX...", 0, 0, 0, 0, 0, 1

Call IniciarDXobject(DirectX)
Call AddtoRichTextBox(frmCargando.status, "Hecho", , , , 1)

AddtoRichTextBox frmCargando.status, " DirectDraw...", 0, 0, 0, 0, 0, 1
Call IniciarDDobject(DirectDraw)
Call AddtoRichTextBox(frmCargando.status, "Hecho", , , , 1)


If Musica = 0 Or Fx = 0 Then
   AddtoRichTextBox frmCargando.status, " DirectSound...", 0, 0, 0, 0, 0, 1
    Call IniciarDirectSound
    Call AddtoRichTextBox(frmCargando.status, "Hecho", , , , 1)
End If

AddtoRichTextBox frmCargando.status, " Placa de Video...", 0, 0, 0, 0, 0, 1
