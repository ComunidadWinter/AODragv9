...
Public Function esLegion(ByVal UserIndex As Integer) As Boolean
'***************************************************
'Autor: Lorwik
'Last Modification: 09/05/2016
'***************************************************
    esLegion = (UserList(UserIndex).Faccion.Legion = 1)
End Function
            ...
            If .TileExit.Map > 0 And .TileExit.Map <= NumMaps Then
                ...
                If UCase$(MapInfo(.TileExit.Map).Restringir) = "NEWBIE" Then
                    ...
                    If EsNewbie(UserIndex) Or EsGM(UserIndex) Then
                        ...
                        End If
                    Else 'No es newbie
                        Call WriteMultiMessage(UserIndex, eMessages.MapaNewbie)
                ...
                ElseIf UCase$(MapInfo(.TileExit.Map).Restringir) = "ARMADA" Then '¿Es mapa de Armadas?
                    ...
                    If esArmada(UserIndex) Or EsGM(UserIndex) Then
                        ...
                        End If
                    Else 'No es armada
                        Call WriteMultiMessage(UserIndex, eMessages.MapaReal)
                        Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
                ...
                ElseIf UCase$(MapInfo(.TileExit.Map).Restringir) = "CAOS" Then '¿Es mapa de Caos?
                    ...
                    If esCaos(UserIndex) Or EsGM(UserIndex) Then
                        ...
                        End If
                    Else 'No es caos
                        Call WriteMultiMessage(UserIndex, eMessages.MapaCaos)
                        Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
                ...
                ElseIf UCase$(MapInfo(.TileExit.Map).Restringir) = "FACCION" Then '¿Es mapa de faccionarios?
                    ...
                    Else 'No es Faccionario
                        Call WriteMultiMessage(UserIndex, eMessages.MapaFaccion)
                        Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
...
'Author: Torres Patricio (Pato)
    ...
    Select Case restrict
        ...
        Case "CLANES"
            RestrictStringToByte = 5
            
        Case "LEGION"
            RestrictStringToByte = 6
...
'***************************************************
    With UserList(UserIndex).Pos
        ...
        If .Map = Casa1 Then Call CasaEncantada(UserIndex)
        
        'Esta en alguno de estos Triggers?
        If MapData(.Map, .X, .Y).trigger = 9 Then
            Call Trampa(UserIndex, 34)
        ElseIf MapData(.Map, .X, .Y).trigger = 10 Then
            Call Trampa(UserIndex, 37)
        End If
    
        'Si el usuario esta en un mapa de desierto le sacamos el gusano?
        If MapInfo(.Map).Terreno = Desierto Then If RandomNumber(1, 100) > 98 Then Call Gusano(UserIndex)
    End With
...
If MapData(MapInvocacion, 19, 40).UserIndex > 0 And MapData(MapInvocacion, 19, 55).UserIndex > 0 And MapData(MapInvocacion, 36, 40).UserIndex > 0 And MapData(MapInvocacion, 36, 55).UserIndex > 0 And MapInfo(MapInvocacion).Invocado = 0 Then
    Call WriteMultiMessage(SendTarget.ToAll, eMessages.Invocacion)
    MapInfo(MapInvocacion).Invocado = 1

    Call SpawnNpc(NPCInvocacion, UserList(MapData(MapInvocacion, XInvocacion, YInvocacion).UserIndex).Pos, True, False)
End If
End Sub
