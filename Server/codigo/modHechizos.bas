...
Public Sub LanzarHechizo(Index As Integer, UserIndex As Integer)
  On Error GoTo errHandler
  
      Dim uh As Integer
      uh = UserList(UserIndex).Stats.UserHechizos(Index)
    
      Dim lanzando As Integer
      lanzando = uh
      Call lanzar_habilidad(lanzando, UserIndex)
    Call WriteCreateParticle(UserIndex, lanzado, 13, 1)
...
errHandler:
  ...
  Debug.Print "Error en LanzarHechizo. Error " & Err.Number & " : " & Err.Description & " UserIndex: " & UserIndex & " Nick: " & UserNick
