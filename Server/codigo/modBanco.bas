...
On Error GoTo Errhandler
    If UserList(UserIndex).Invent.Object(Item).Amount > 0 And Cantidad > 0 Then
        ...
        '<Edurne>
        If ObjData(Item).IndiceMontura And UserList(UserIndex).flags.QueMontura Then _
            Call WriteConsoleMsg(UserIndex, "No puedes depositar una montura estando montado.", FontTypeNames.FONTTYPE_INFO): Exit Sub
        '</Edurne>
